...
external = "./FCPGs/"
print(external)
years = range(2004, 2021, 1)
#years = range(int(sys.argv[2]), int(sys.argv[3]), 1)
#print(years)
dirs = os.listdir(os.path.join(external, param))
print(dirs)
for year in years:
	to_merge = []
	for dir in dirs:
		files = os.listdir(os.path.join(external, param ,dir))
		for f in files:
			if str(year) in f:
				ras = rasterio.open(os.path.join(external, param, dir, f))
				to_merge.append(ras)
	print(to_merge)
